...
class GradientInversionAttack:
    ...
    def attack(self, original_data: torch.Tensor, true_grad: torch.Tensor,
        ...
        for step in range(1, iterations + 1):
            ...
            # Compute gradients w.r.t. model params using backward to keep compatibility
            loss.backward(create_graph=True, retain_graph=True)
            grads_flat = [p.grad.flatten() for p in self.model.parameters() if p.grad is not None]
            if len(grads_flat) == 0:
                break
...
class ViewAugmenter:
    ...
    def __init__(
        ...
        color_jitter: Tuple[float, float, float, float] = (0.6, 0.6, 0.6, 0.2),
    ):
        self.num_views = num_views
        self.mask_ratio = mask_ratio
        ...
        if self.apply_transforms:
            ...
            self.view_transform = transforms.Compose([
                ...
                transforms.RandomPerspective(distortion_scale=0.3, p=self.perspective_prob),
                ...
                transforms.RandomApply([
                    transforms.RandomSolarize(threshold=self.solarize_threshold)
