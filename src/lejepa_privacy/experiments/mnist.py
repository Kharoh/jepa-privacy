    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.spatial_transform = v2.RandomAffine(
            ...
            translate=(
                kwargs.get("translation_px", 3) / 28,
                kwargs.get("translation_px", 3) / 28,
            ),
...
def compute_loss_components(
    ...
    batch: torch.Tensor,
    model_type: str,
    augmenter: "ViewAugmenter" | None = None,
) -> Dict[str, float]:
    ...
    def local_train(self, global_model: nn.Module, epochs: int, lr: float) -> Dict:
        ...
        if self.dp_config.enabled and self.dp_config.apply_to_updates:
            with torch.no_grad():
                ...
                dp_delta_values, _ = apply_dp_to_tensors(
                    delta_values, self.dp_config, self.dp_generator
                )
                dp_delta_state = {
                    k: dp_delta_values[i] for i, k in enumerate(delta_state.keys())
                }
...
def run_federated_privacy_experiment() -> Dict[str, Dict[str, List[float]]]:
    """
    ...
    if dp_config.enabled:
        print(
            f"DP Mode: enabled | clip_norm={dp_config.clip_norm} | "
    ...
    class_counts = plot_client_class_distribution(
        client_labels, num_classes=10, save_path="client_class_distribution.png"
    )
    for c in range(NUM_CLIENTS):
        print(
            f"  Client {c}: samples={len(client_data[c])}, "
            f"class_counts={class_counts[c].tolist()}"
        )
    ...
    for round_idx in range(NUM_ROUNDS):
        ...
        lejepa_updates = [
            client.local_train(lejepa_server.global_model, epochs=2, lr=1e-4)
        ...
        mae_updates = [
            client.local_train(mae_server.global_model, epochs=2, lr=1e-4)
        ...
        if round_idx % EVAL_EVERY == 0 or round_idx == NUM_ROUNDS - 1:
            ...
            if round_idx == NUM_ROUNDS - 1:
                ...
                last_reconstructions["lejepa_mse"] = (
                    x_test_views.detach(),
                    lejepa_recon_mse.detach(),
                )
